...
if st.session_state.running and current_step < len(t) - 1:
    while st.session_state.running and current_step < len(t) - 1:
        ...
        # 更新曲线高亮（可选：添加当前时刻竖线）
        for ax, fig in zip([ax_p, ax_ke, ax_v, ax_x], [fig_p, fig_ke, fig_v, fig_x]):
            for line in ax.lines:
                if len(line.get_xdata()) > 1 and len(line.get_ydata()) > 1:
                    line.set_data(t[:current_step+1], line.get_ydata()[:current_step+1])
            # 添加当前时刻竖线
            for patch in ax.patches:
                if hasattr(patch, 'get_label') and patch.get_label() == 'current_time':
                    patch.remove()
            ax.axvline(x=t[current_step], color='gray', linestyle='--', alpha=0.5, label='current_time')
            fig.canvas.draw()

        p_placeholder.pyplot(fig_p)
        ke_placeholder.pyplot(fig_ke)
        v_placeholder.pyplot(fig_v)
        x_placeholder.pyplot(fig_x)
